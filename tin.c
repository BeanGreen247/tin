...
enum KEY_ACTION{
        ...
        /* The following are just soft codes */
        ARROW_LEFT = 1000,
        ARROW_RIGHT,
        ARROW_UP,
        ARROW_DOWN,
...
char *Python_keywords[] = {
	...
	/* Python types */
        "int|","complex|","float|","str|","list|","tuple|",
        "range|","dict|","set|","frozenset|","bool|","bytes|",
...
int getWindowSize(int ifd, int ofd, int *rows, int *cols) {
    ...
    /* Restore position. */
    char seq[512];
    snprintf(seq,512,"\x1b[%d;%dH",orig_row,orig_col);
    ...
    if (ioctl(1, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
      ...
      *rows = ws.ws_row;
      //updateWindowSize();
      //retval = getCursorPosition(ifd,ofd,ws.ws_row,ws.ws_col);
      //if (retval == -1) {
      //*cols = ws.ws_col;
      //*rows = ws.ws_row;
      //updateWindowSize();
...
void editorUpdateRow(erow *row) {
   ...
   /* Create a version of the row we can directly print on the screen,
    ...
    if (allocsize > UINT32_MAX) {
        printf("Some line of the edited file is too long for tin\n");
        exit(1);
    }
...
#define tin_QUERY_LEN 256

void editorFind(int fd) {
    char query[tin_QUERY_LEN+1] = {0};
    int qlen = 0;
...
void editorProcessKeypress(int fd) {
    ...
    quit_times = tin_QUIT_TIMES; /* Reset it to the original value. */
}
...
void updateWindowSize(void) {
    ...
    E.screencols -= 2; /* Get room for y axis*/
}
...
int main(int argc, char **argv) {
    if (argc != 2) {
        fprintf(stderr,"Usage: tin <filename>\n");
        exit(1);
    }
